...
# prepares the term_term_id_frequency and doc_term_id_frequency of the entire vocabularyentire vocabulary
...
def prepareFeatureVector(p_term_id_frequency_list):
    ...
    if type(p_term_id_frequency_list) is str:
        ...
        p_term_id_frequency_list = \
            ast.literal_eval(p_term_id_frequency_list)
...
def measureDistance(from_vector, to_vector):
    ...
    result = numpy.linalg.norm(numpy.array(to_vector)
                               - numpy.array(from_vector))
...
def initialize(p_class_id_map):
    ...
    for train_doc_id in doc_id_term_id_map:
        training_feature_vector[train_doc_id] = \
            prepareFeatureVector(doc_id_term_id_map[train_doc_id])
        ...
        for test_document in test_document_list:
            ...
            for term in token_results['set']:
                ...
                if term in token_results['title']:
                    ...
                    term_frequency = 3 * token_results['title'
                            ].count(term)
                else:
            ...
            test_feature_vector = \
                prepareFeatureVector(term_id_frequency_list)
            ...
            for class_item in class_set:
                ...
                class_frequency_map[class_item] = \
                    class_list.count(class_item)

                # if class_item in doc_id_class_id_list[test_doc_id]:
                # ........correctly_matched[test_doc_id] = 1

            sorted_class_frequency = \
                sorted(class_frequency_map.iteritems(),
            ...
            print 'Classes for Test Document ', test_doc_id, \
                ' might be one of the following ', class_names
    ...
    print "Program's total execution time is ", endProgram \
        - startProgram, ' s'
